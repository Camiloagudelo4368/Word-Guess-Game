...
function GetCity() {
    ...
    // if(numbernorepeat.indexOf(_randomId) === -1){
...
function start() {
    GetCity();
    // numbernorepeat.push(_cityResult.id);
    ...
    if (_citiesDone.indexOf(_cityResult.id) !== -1) {
        ...
        var _description = document.getElementById("imgText");
        var _cityName = document.getElementById("cityName");
...
function win() {
    ...
    _wordControl = 1;
    $("#cityName").text(_cityResult.name);
    $("#imgText").text(_cityResult.description);
...
document.onkeyup = (function (event) {
    ...
    if (_winFlag === 0) {
        ...
        if (_start !== 0) {
            ...
            if (_index !== -1) {
                ...
                if ($("#currentWord").text().indexOf(_key.toUpperCase()) !== -1) {
                    ...
                    for (var _i = 0; _i <= _cityResult.key.length; _i++) {

                        if ($("#currentWord").text().indexOf(_i) === -1) {

                            if (_cityResult.word.indexOf(_key) !== -1) {
                                if (_cityResult.word[_i] === _key) {
        ...
        // confirm("...Congratulations... continue");
        start();
        _start = 1;
        _winFlag = 0;
        $("#pressAny").text("");
    ...
    if (_winFlag === 0) {
        if (_cityResult.key === ($("#currentWord").text())) {
            win();            
        }
    }
